...
func BfsHelper(a *RecipeTreeNode) {
	...
	if a.Parent == nil {
		...
		if a.Parent.Parent != nil { // Pastikan kakek tidak nil
			...
			if a.Parent.Parent.Parent != nil { // Parent dari kakek adalah *RecipeTreeNodeChild
				...
				log.Printf("[BFS_HELPER_DEBUG] Node kakek ('%s') memiliki parent (RecipeTreeNodeChild). LeftChild kakek-parent: %s, RightChild kakek-parent: %s",
					parentParentNodeName,
					a.Parent.Parent.Parent.LeftChild.NamaElemen,  // Asumsi tidak nil
					a.Parent.Parent.Parent.RightChild.NamaElemen) // Asumsi tidak nil
			} else {
	...
	for a.Parent != nil { // Jika a.Parent awalnya nil, loop ini tidak akan berjalan.
		...
		log.Printf("[BFS_HELPER_DEBUG] Iterasi ke-%d loop. Node 'a': '%s'. Parent-RTC-nya ('%p') adalah anak dari '%s'. Kakek-RTC-nya ('%p') adalah anak dari '%s'",
			iterCount, a.NamaElemen, a.Parent, currentParentNodeName,
			a.Parent.Parent.Parent,     // Ini adalah RecipeTreeNodeChild "buyut"
			currentGrandParentNodeName) // Ini nama elemen dari parent si "buyut RTC"
...
func IsBFSPath(a *RecipeTreeNodeChild) bool {
	...
	log.Printf("[IS_BFS_PATH_DEBUG] Memeriksa path untuk Parent '%s', Left '%s'(Resep:%d, Basic:%t), Right '%s'(Resep:%d, Basic:%t). Hasil: %t",
		a.Parent.NamaElemen,
		a.LeftChild.NamaElemen, a.LeftChild.BanyakResep, IsBasicElement(*a.LeftChild),
		a.RightChild.NamaElemen, a.RightChild.BanyakResep, IsBasicElement(*a.RightChild),
		result)
	return result
}
...
func IsValid(idParent int, idLeftChild int, idRightChild int) bool {
	...
	log.Printf("[IS_VALID_DEBUG] Data untuk IsValid: Parent (diakses pada indeks %d): '%s' (ID %d, Tier %d), Left (diakses pada indeks %d): '%s' (ID %d, Tier %d), Right (diakses pada indeks %d): '%s' (ID %d, Tier %d)",
		idParent, elParent.Name, elParent.ID, elParent.Tier,
		idLeftChild, elLeft.Name, elLeft.ID, elLeft.Tier,
		idRightChild, elRight.Name, elRight.ID, elRight.Tier)

	result := elParent.Tier > elLeft.Tier && elParent.Tier > elRight.Tier
	log.Printf("[IS_VALID_DEBUG] Hasil perbandingan Tier: (%d > %d && %d > %d) -> %t",
		elParent.Tier, elLeft.Tier, elParent.Tier, elRight.Tier, result)
	return result
}
...
func Bfs(start int, needFound int, mult int) *RecipeTreeNode {
	...
	for i := 0; i < len(elements[start].FromPair); i++ {
		...
		if IsValid(start, leftChildID_fromPair, rightChildID_fromPair) {
			...
			log.Printf("[BFS_DEBUG] [InitQueue] Menambahkan ke queue: Parent='%s', Left='%s'(ID:%d), Right='%s'(ID:%d). Queue size: %d",
				childRelation.Parent.NamaElemen, tempLeftChildNode.NamaElemen, leftChildID_fromPair, tempRightChildNode.NamaElemen, rightChildID_fromPair, len(queue))
		} else {
	...
	for len(queue) > 0 {
		...
		current := queue[0]
		queue = queue[1:]
		log.Printf("[BFS_DEBUG] [Loop %d] Mengambil dari queue: Parent='%s', LeftChild='%s'(ID:%d), RightChild='%s'(ID:%d). Sisa queue: %d",
			processedInQueue, current.Parent.NamaElemen, current.LeftChild.NamaElemen, current.LeftChildID, current.RightChild.NamaElemen, current.RightChildID, len(queue))
